...
import re
import math
from .utils import get_smiles,get_cid_from_smiles, get_pubchem_description, get_pubchem_record_sections


def has_a_smell(compound_name_or_smiles):
    smiles = get_smiles(compound_name_or_smiles) if not compound_name_or_smiles.startswith("C") else compound_name_or_smiles
...
def evaporation_trace(smiles_or_name: str, save_path: str = "evaporation_curve.png"):
    try:
        if not smiles_or_name.startswith("C"): 
            smiles = get_smiles(smiles_or_name)
        else:
